...
## Supabase Realtime setup for Gomoku

Gomoku now uses Supabase Realtime for cross-device room sync. GitHub Pages still hosts the frontend, and Supabase provides the realtime room channel.
...
The anon key is designed for browser use. Keep Row Level Security and project policies configured properly if you later add database tables. The current Gomoku implementation uses Realtime broadcast channels for transient room messages.
